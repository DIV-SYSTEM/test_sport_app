...
class LocationFilterService {
  ...
  Future<List<TravelModel>> filterByDistance(double maxDistanceKm) async {
    if (maxDistanceKm == 0) {
      return travelData; // No distance filter applied
    }

    final userLocation = await getUserLocation();
    if (userLocation == null) {
      return travelData; // Return all if location unavailable
    }

    return travelData.where((companion) {
