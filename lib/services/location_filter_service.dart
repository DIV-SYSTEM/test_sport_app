...
class LocationFilterService {
  ...
  Future<List<CompanionModel>> filterByDistance(double maxDistanceKm) async {
    if (maxDistanceKm == 0) {
      return companionData; // No distance filter applied
    }

    final userLocation = await getUserLocation();
    if (userLocation == null) {
      return companionData; // Return all if location unavailable
    }

    return companionData.where((companion) {
